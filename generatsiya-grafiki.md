...
3. **Масштаб**: для предметов — один коэффициент на всё, считается по эталону (во сколько раз уменьшить эталон, чтобы тело стало 120 px); предметы нарисованы «в масштабе референса» и уменьшаются так же, а не подгоняются под холст. **Части тела — наоборот, поштучно под канон рига**: рука ужимается до 40 px от плеча, нога до 34, иначе суставы не сойдутся. Модель при разбивке ошибается в длине конечностей на 20–30%, и это штатно.
...
Сделано для частей персонажа (шаги 1–5). Предметы — следующий этап: тот
же скрипт, отдельная команда, общий коэффициент масштаба по эталону.
