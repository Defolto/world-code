...
**Статистика прохождений** — http://127.0.0.1:5173/stats/, пароль в деве
`dev` (переменная `STATS_PASSWORD`; нужен запущенный бэкенд, то есть
`npm run dev`, а не `dev:web`). Каждая попытка —
победа или уход со страницы после хотя бы одного запуска — уезжает на
`POST /api/attempts` и дописывается строкой в `backend/data/attempts.jsonl`
(`DATA_DIR`, в git не попадает): все запуски с кодом и ошибками, время,
итоговый код, `passed`, `version` — хэш уровня из сборки. Страница
сводит это по уровням и подсвечивает подозрительно простые и проблемные —
в том числе те, что бросают. Правка карты, заготовки или условия меняет
версию: попытки на прежней редакции показываются отдельной строкой под
текущей и в её вердикт не попадают.
...
<корень сайта>/
  ...
  data/              попытки пройти уровни (attempts.jsonl); создаётся сам
...
имя файла — имя переменной, содержимое — значение. Нужна одна:
`STATS_PASSWORD` — пароль к `/stats/`; без неё страница и `/stats/data`
отвечают 503, запись попыток при этом работает.
